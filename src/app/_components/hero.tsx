...
function Hero(): React.JSX.Element {
  return (
    <section data-testid="hero" className="flex  flex-col gap-y-8 py-20 text-center">
      ...
      <div className="flex flex-row items-center justify-center gap-x-3">
        <Link href={"/docs"}>
          <Button size={"lg"}>
            Documentation
            <Atom data-testid="docs" className="ml-2 h-4 w-4" />
          </Button>
        </Link>
        <Link href={siteConfig.links.github}>
          <Button variant={"outline"} size={"lg"}>
            Github
            <Github data-testid="github" className="ml-2 h-4 w-4" />
          </Button>
        </Link>
      </div>
    </section>
